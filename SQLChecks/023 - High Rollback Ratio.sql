/* DESCRIPTION:
   What This Means: High rollback ratio detected in this database. A significant
   percentage of transactions are being rolled back instead of committed.

   WHY THIS MATTERS:
   A high rollback ratio can indicate application errors, failed transactions,
   deadlocks, lock timeouts, statement timeouts, constraint violations, retry
   behavior, or inefficient transaction handling. Beyond correctness concerns,
   frequent rollbacks waste work already performed and add unnecessary load.
   ...
   violations, and application retry patterns. If the rollback volume is
   expected, document the reason and validate that it does not create
   unnecessary load.
...
(
    ...
    WHERE datname = current_database()
      ...
      AND xact_rollback::numeric
          / NULLIF(xact_commit + xact_rollback, 0) * 100 > 10
);
